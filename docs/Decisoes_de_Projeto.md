# Decisões de Projeto e Premissas - Meu Vale FGV
...
*   **Conciliação Visual:** Para evitar o tráfego de documentos sensíveis dos usuários (como upload de contracheques), a conciliação será feita visualmente pelo próprio colaborador, confrontando os relatórios gerados pelo "Meu Vale" com os dados do seu contracheque.
...
*   **Nomenclatura Profissional:** O aplicativo manterá um nome padrão e sério (como "Meu Vale FGV"), descartando sugestões de nomes cômicos ("Saporrê", "Mate do Rei") para preservar a credibilidade da ferramenta em uma eventual expansão de uso.
*   **Tipos de Refeição Flexíveis:** A base de dados e a interface devem prever as diferentes regras do RH, permitindo lançar não apenas a "Refeição a Quilo", mas também o "Prato Feito" (prato básico sem peso, com valor único) e itens com custo zero (como a sobremesa/fruta já incluída no valor do prato básico).
